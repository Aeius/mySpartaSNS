...
def detail_tweet(request, id):
    if request.method == "GET":
        ...
        return render(request, 'tweet/tweet_detail.html', {'tweet': selected_tweet, 'comment': comment})


def write_comment(request, id):
    if request.method == "POST":
        user = request.user
        comment_give = request.POST.get('comment', '')
        current_tweet = TweetModel.objects.get(id=id)

        comment = TweetComment()
        comment.comment = comment_give
        comment.author = user
        comment.tweet = current_tweet
        comment.save()
